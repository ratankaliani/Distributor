...
contract Pool is Exponential {
    ...
    constructor (uint _poolID, address _organization, address _lendingPool, uint _timeToClose, uint _bondPeriod) {
        poolID = _poolID;
    ...
    function withdraw(address asset, address lpAsset, uint amount) public {
        require(finalized == 1, "Pool not finalized");
    ...
    */
    function finalize(address assetAddress) public returns (address[] memory, uint[] memory, uint) {
